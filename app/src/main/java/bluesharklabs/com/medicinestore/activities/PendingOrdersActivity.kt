...
import bluesharklabs.com.medicinestore.adapters.ViewAllOrderAdapter
...
class PendingOrdersActivity : AppCompatActivity(), View.OnClickListener, SwipeRefreshLayout.OnRefreshListener {

    var Orderdata: List<ViewallStoreOrderResponse.Datum> = arrayListOf()

    private var ordersAdapter: ViewAllOrderAdapter? = null
    private var recycler_orders: RecyclerView? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent != null) {
            if (intent.getStringExtra("direct") != null) {
                txt_title.setText(R.string.direct_orders)
                direct = "1"
                statusViewall= intent.getStringExtra("direct")
            }

            if (intent.getStringExtra("pending") != null) {
                txt_title.setText(R.string.pending_orders)

                statusViewall= intent.getStringExtra("pending")
            }
        }
       ...
       simpleSwipeRefreshLayout.post(Runnable {
        ...
        recycler_orders = findViewById(R.id.recycl_ordrs)

        recycler_orders!!.setHasFixedSize(true)
        recycler_orders!!.layoutManager =
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                   simpleSwipeRefreshLayout.isRefreshing = false
                    ...
                    ordersAdapter = ViewAllOrderAdapter(this@PendingOrdersActivity, Orderdata,statusViewall,direct)
                    ...
                    try {
                        ...
                        Toast.makeText(this@PendingOrdersActivity, e.message, Toast.LENGTH_LONG).show()
                    }
